    ...
    def update(self):
        ...
        for virus in self.viruses:
            if not virus.doesClear():
                survive.append(virus)
        self.viruses = survive
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for index in range(100):
        viruses.append(SimpleVirus(maxBirthProb=.1, clearProb=.05))
    p = Patient(viruses, maxPop=1000)
    ...
    pylab.title('Virus population (no drugs)')
    pylab.show()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    # Generate stable of numTrials patients
    patient_list = []
    for i in range(numTrials):
        # Generate stable of numViruses viruses for each patient
        virus_list = []
        for j in range(numViruses):
            virus_list.append(ResistantVirus(maxBirthProb, clearProb,
                                             resistances, mutProb))
        patient_list.append(TreatedPatient(virus_list, maxPop))
 
    avg_virus_counts = []
    avg_resistant_counts = []
    stepCount = 0
 
    # Run without medication for 150 time steps
    while stepCount < 150:
        stepCount += 1
        avg = 0
        res = 0
        for patient in patient_list:
            patient.update()
            avg += patient.getTotalPop()
            res += patient.getResistPop('guttagonol')
        avg /= float(len(patient_list))
        # Record average total populations
        avg_virus_counts += [avg]
        res /= float(len(patient_list))
        # Record average resistant populations
        avg_resistant_counts += [res]
 
    # Administer medicine to patients
    for patient in patient_list:
        patient.addPrescription('guttagonol')
 
    # Run for another 150 steps
    while stepCount < 300:
        stepCount += 1
        avg = 0
        res = 0
    ...
    pylab.figure(1)
    pylab.plot(avg_virus_counts, label='Total Virus Avg')
    pylab.plot(avg_resistant_counts, label='Resistant Virus Avg')
