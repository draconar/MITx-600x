...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        else: self.setRobotDirection(random.randrange(360))
    ...
    for n in range(num_trials):
        ...
        while True:
            ...
            if (float(room.getNumCleanedTiles()) / float(room.getNumTiles())) >= min_coverage:
                iList.append(i)
                break
    return float(sum(iList)/(len(iList)))
...
#     plot.
#
#showPlot1('whatever the title is', 'whatever x label is', 'whatever y label is')
#
...
#     plot.
#
showPlot2('whatever the title is', 'whatever x label is', 'whatever y label is')
#
#
